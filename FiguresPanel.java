...
public class FiguresPanel extends JPanel {
	...
	public void paint( final Graphics drawingTool ) {
		...
		for ( final Figure figure : controler.getCanvasFigures() ) {
			...
			final Color fillColor = mapColor( figure.getStyle() );
	...
	private Color mapColor( final FigureStyle figureStyle ) {
		if ( figureStyle.getRedColor() < 0 || figureStyle.getGreenColor() < 0 || figureStyle.getBlueColor() < 0 ) {
			return null;
		}
		
		return new Color( figureStyle.getRedColor(), figureStyle.getGreenColor(), figureStyle.getBlueColor() );
	}
